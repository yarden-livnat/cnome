...
import edu.utah.sci.cyclist.ui.components.RangeSliderSkin;
...
public class RangeSlider extends Control {
    ...
    @Override protected String getUserAgentStylesheet() {
        //return getClass().getResource("assets/rangeslider.css").toExternalForm();
    	return Cyclist.class.getResource("assets/rangeslider.css").toExternalForm();
    }
    ...
    private DoubleProperty lowValue = new SimpleDoubleProperty(this, "lowValue", Double.MIN_VALUE/*0.0D*/) {
    ...
    public final double getLowValue() {
    	...
    	return lowValue != null ? lowValue.get() : getMin();
    }
    ...
    private DoubleProperty highValue = new SimpleDoubleProperty(this, "highValue", Double.MAX_VALUE/*100D*/) {
        @Override protected void invalidated() {
            adjustHighValues();
        }
    ...
    public final double getHighValue() {
    	...
    	return highValue != null ? highValue.get() : getMax();
    }
    ...
    public final double getMax() {
    	...
    	return max == null ? Double.MAX_VALUE : max.get();
    }

    public final DoubleProperty maxProperty() {
        if (max == null) {
            max = new DoublePropertyBase( Double.MAX_VALUE /*100*/) {
                @Override protected void invalidated() {
                    if (get() < getMin()) {
                        setMin(get());
                    }
    ...
    public final double getMin() {
        return min == null ? Double.MIN_VALUE : min.get();
    }

    public final DoubleProperty minProperty() {
        if (min == null) {
            min = new DoublePropertyBase(Double.MIN_VALUE /*0*/) {
                @Override protected void invalidated() {
                    if (get() > getMax()) {
                        setMax(get());
                    }
