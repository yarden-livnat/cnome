	...
	private void init (){
		...
		facilityList.valueProperty().addListener(new ChangeListener<String>(){
			...
			public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
				...
				data.clear();
				int i=0;
				while (!(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).name.equals(newValue))){
					i++;
				}
				...
				for (int ii=0;ii<CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityStructure.size();ii++){
					...
					for (int iii=0;iii<CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.size();iii++){
						element.add(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.get(iii).facilityData.get(ii));
					}
					data.add(element);
				}
				for (int iii=0;iii<CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.size();iii++){
					display.getChildren().add(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.get(iii).sorterCircle);
					display.getChildren().add(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.get(iii).sorterCircle.text);
				}
				number.setVisible(true);
				number.setText("number of nodes shown:" +Integer.toString(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.size()));

				//update information in searchbox
				type.setText("Type: "+CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityType);
				
				numberShowen.setMax(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityClones.size());
				...
				numberShowen.valueProperty().addListener(new ChangeListener<Number>(){
		            public void changed(ObservableValue<? extends Number> ov,Number old_val, Number new_val) {
		                   ...
		                   for (int iii=0;iii<value;iii++){
		   					display.getChildren().add(CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(iii).sorterCircle);
		   				}
			...
			//	add.setId(Integer.toString(i));
				...
				formSort(CycicScenarios.workingCycicScenario.FacilityNodes.get(i).facilityStructure,data,structure);
		...
		//		System.out.println(data);
				...
				start.setOnMouseClicked(new EventHandler <MouseEvent>(){
					...
					public void handle(MouseEvent event) {
						...
						for (int ii=0;ii<CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.size();ii++)
						{
							facilityNode dataarray=CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(ii);
							for (int iv=0;iv<CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityStructure.size();iv++){
								if (!(((ArrayList) CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityStructure.get(iv)).get(1) instanceof ArrayList)){
									struc.add((ArrayList) CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityStructure.get(iv));
								} else {
									ArrayList<ArrayList> subtruc =(ArrayList) CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityStructure.get(iv);
				...
				//logic error occur
							...
							if (truth==true){
								displaylist.add(CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(ii));
								display.getChildren().add(CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(ii).sorterCircle);
								display.getChildren().add(CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(ii).sorterCircle.text);
								display.getChildren().add(CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(ii).sorterCircle.menu);
							}
							
						}
						System.out.println(displaylist.toString());
						System.out.println(struc);
						System.out.println(CycicScenarios.workingCycicScenario.FacilityNodes.get(order).facilityClones.get(0).facilityData);
					}
				});
