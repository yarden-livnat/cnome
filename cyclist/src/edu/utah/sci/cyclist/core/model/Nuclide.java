...
public class Nuclide implements CyclistData {
	...
	static public int zzaaam(int nuc) throws NotNuclide, IndeterminateNuclideForm {
		int nucid = id(nuc);
	...
	static public int zzzaaa(int nuc) throws NotNuclide, IndeterminateNuclideForm {
		return id(nuc)/10000;
	}
	...
	public static int id(int nuc) throws NotNuclide, IndeterminateNuclideForm {
		...
		//else if (aaassss == 0 && 0 == _zz_name.containsKey(nuc/1000) && 0 < _zz_name.containsKey(zzz))
		else if (aaassss == 0 && _zz_name.containsKey(zzz)) {
	...
	//	static public int id(String nuc) {
	//		if (nuc.isEmpty())
	...
	//		    if( nuc.substring(1, 3).contains("-") && nuc.substring(4,  5).contains("-")) {
	...
	//
	//		    if (pyne::contains_substring(pyne::digits, nucstr.substring(0, 1))) {
	//		      if (pyne::contains_substring(pyne::digits, nucstr.substring(nuclen-1, nuclen))) {
	...
	//		      };
	//		    } else if (pyne::contains_substring(pyne::alphabet, nucstr.substring(0, 1))) {
	...
	//		      String end_char = pyne::last_char(nucstr);
	//		      if (end_char == "M")
	//		        newnuc = (10000 * anum) + 1;
	...
	//
	//		      // Add the Z-number
	//		      elem_name = pyne::remove_characters(nucstr.substring(0, nuclen-1), pyne::digits);
	//		      elem_name = pyne::capitalize(elem_name);
	//		      if (0 < name_zz.count(elem_name))
	//		        newnuc = (10000000 * name_zz[elem_name]) + newnuc;
	//		      else
	...
	//		  };
	//	}
	public static String name(int nuc) throws NotNuclide, IndeterminateNuclideForm {
		int nucid = id(nuc);
		String newnuc = "";
		...
		if (!_zz_name.containsKey(zzz))
			throw new NotNuclide(nuc); // NotNuclide(nuc, nucid);
	...
	// Lanthanides
	static public List<String> LAN_IDS = Arrays.asList("La", "Ce", "Pr", "Nd", "Pm", "Sm", "Eu", "Gd", "Tb", "Dy", "Ho", "Er", "Tm", "Yb", "Lu");
	static public Set<String> LAN = new HashSet<>(LAN_IDS);
	static public Set<Integer> lan; 

	// Actinides
	static public List<String> ACT_IDS = Arrays.asList("Ac", "Th", "Pa", "U", "Np", "Pu", "Am", "Cm", "Bk", "Cf", "Es", "Fm", "Md", "No", "Lr");
	static public Set<String> ACT = new HashSet<>(ACT_IDS);
	static public Set<Integer> act; 

	// Transuarnics
	static public List<String> TRU_IDS = Arrays.asList("Np", "Pu", "Am", "Cm", "Bk", "Cf", "Es", "Fm", "Md", "No", "Lr", "Rf", "Db", "Sg", "Bh", 
			"Hs", "Mt", "Ds", "Rg", "Cn", "Fl", "Lv");
	static public Set<String> TRU = new HashSet<>(TRU_IDS);
	static public Set<Integer> tru;

	//Minor Actinides
	static public List<String> MA_IDS = Arrays.asList("Np", "Am", "Cm", "Bk", "Cf", "Es", "Fm", "Md", "No", "Lr");
		...
		_fluka_zz = new HashMap<String, Integer>() {
			{
				...
				put("UNUNQUAD", 1140000000);  // no fluka:  UNUNQUADIUM,  "Flerovium"
				put("UNUNHEXI", 1160000000);  // no fluka:  UNUNHEXIUM , "Livermorium" 
...
class NotNuclide extends Exception {
	private int _nuc;

	public NotNuclide(int nuc) {
		this(nuc, "");
	}

	public NotNuclide(int nuc, String msg) {
		super(msg);
		_nuc = nuc;
	}

	public NotNuclide(String nuc, String string) {
...
class IndeterminateNuclideForm extends Exception {
	private int _nuc;
